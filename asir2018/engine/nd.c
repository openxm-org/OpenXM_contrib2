...
INLINE int ndl_reducible(UINT *d1,UINT *d2)
{
    ...
    for ( i = nd_exporigin; i < nd_wpd; i++ ) {
        u1 = d1[i]; u2 = d2[i];
        u1 -= u2;
        if ( u1 & nd_mask1 ) return 0;
    }
#endif
#if 0
    for ( i = nd_wpd-1; i >= nd_exporigin; i-- ) {
        u1 = d1[i]; u2 = d2[i];
        u1 -= u2;
        if ( u1 & nd_mask1 ) return 0;
    }
    ...
    i = nd_exporigin;
    while ( nd_wpd-i >= 4 ) {
       u1 = d1[i]-d2[i]; if ( u1 & nd_mask1 ) return 0; i++;
       u1 = d1[i]-d2[i]; if ( u1 & nd_mask1 ) return 0; i++;
       u1 = d1[i]-d2[i]; if ( u1 & nd_mask1 ) return 0; i++;
       u1 = d1[i]-d2[i]; if ( u1 & nd_mask1 ) return 0; i++;
    }
...
// compare t and s for module monomials t*ei and s*ej 
...
INLINE int ndl_find_reducer(UINT *dg)
{
    ...
    if ( Reverse > 0 )
        ...
        // find the shortest diviser
        int minlen = 0, mini = -1;
        for ( i = 0; i < nd_psn; i++ ) {
            r = nd_psh[i];
            if ( ndl_reducible(dg,DL(r)) ) {
                if ( mini < 0 || r->len < minlen ) {
                  mini = i; minlen = r->len;
                }
            }
        }
...
NMs nms_merge(NMs p1,NMs p2)
{
    ...
    if ( !p1 ) return p2;
        ...
        can = 0;
        for (  m1 = p1, m2 = p2, mr0 = 0; m1 && m2; ) {
            c = DL_COMPARE(DL(m1),DL(m2));
            switch ( c ) {
                case 0:
                    c = comp_sig(m1->sig,m2->sig);
                    if ( c > 0 ) {
                      s = m2;
                    } else {
                      s = m1; 
                    }
                    m1 = NEXT(m1); m2 = NEXT(m2);
                    can++; NEXTNM2(mr0,mr,s);
                    break;
                case 1:
                    s = m1; m1 = NEXT(m1); NEXTNM2(mr0,mr,s);
...
SGeoBucket create_sbucket()
{
    SGeoBucket g;
     
    g = CALLOC(1,sizeof(struct oSGeoBucket));
    g->m = -1;
    return g;
}
...
void add_sbucket(SGeoBucket g,NMs d)
{
  int l,k,m,m0;
...
NMs remove_head_sbucket(SGeoBucket g)
{
    int j,i,k,c;
    NMs head;
    ...
    for ( i = 0; i <= k; i++ ) {
        ...
        if ( j < 0 ) j = i;
            ...
            c = DL_COMPARE(DL(g->body[i]),DL(g->body[j]));
            if ( c > 0 )
                j = i;
            else if ( c == 0 ) {
                c = comp_sig(g->body[i]->sig,g->body[j]->sig);
                if ( c > 0 )
    ...
    if ( SG(l1) != sugar ) {
      ...
      for ( i = 0; i < nd_nbase; i++ )
        for ( t = d[i]; t; t = NEXT(t) )
          if ( t->sugar == sugar ) k++;
      if ( DP_Print) fprintf(asir_out,"(%d)",k);
    }
    sig = l1->sig;
...
get_eg(&eg3); add_eg(&eg_merge,&eg2,&eg3);
  if ( Nnewpair < 0 ) fprintf(asir_out,"{%d}",Nnewpair);
  return Nnewpair;
}
...
ND_pairs *nd_minsugar_array_s( ND_pairs *d, int sugar)
{
  ...
  r = (ND_pairs *)CALLOC(nd_nbase,sizeof(ND_pairs));
  min = -1;
  for ( i = 0; i < nd_nbase; i++ )
    if ( d[i] && (min < 0 || d[i]->sugar < min) ) min = d[i]->sugar;
  if ( sugar >= 0 ) {
    if ( sugar > min ) return r;
    else min = sugar;
  }
  for ( i = 0; i < nd_nbase; i++ ) {
    if ( d[i] == 0 ) continue;
...
void nd_setup_parameters(int nvar,int max) {
    ...
    if ( max ) {
        ...
        else if ( max < 2 ) nd_bpe = 2;
        else if ( max < 4 ) nd_bpe = 3;
        else if ( max < 8 ) nd_bpe = 4;
        else if ( max < 16 ) nd_bpe = 5;
        else if ( max < 32 ) nd_bpe = 6;
        else if ( max < 128 ) nd_bpe = 8;
        else if ( max < 512 ) nd_bpe = 10;
        else if ( max < 32768 ) nd_bpe = 16;
        else nd_bpe = 32;
    }
...
ND_pairs nd_reconstruct(int trace,ND_pairs d)
{
    ...
    for ( i = nd_psn-1; i >= 0; i-- ) {
        ...
        tmp = (UINT *)MALLOC(nd_wpd*sizeof(UINT));
        ndl_reconstruct(nd_bound[i],tmp,obpe,oepos);
        nd_bound[i] = tmp;
    }
    if ( trace )
        for ( i = nd_psn-1; i >= 0; i-- ) {
...
int nd_sp_lr(int mod,int trace,ND_pairs p,ND *left,ND *right)
{
    NM m1,m2;
    NDV p1,p2;
    ND t1,t2;
    ...
    *left = ndv_mul_nm(mod,m1,p1); *right = ndv_mul_nm(mod,m2,p2);
    return 1;
}
...
int ndv_reduce_vect_small(int m,UINT *svect,int col,IndArray *imat,NM_ind_pair *rp0,int nred,SIG sig)
{
    int i,j,k,len,pos,prev;
    UINT c,c1,c2,c3,up,lo,dmy,t;
    IndArray ivect;
    unsigned char *ivc;
    unsigned short *ivs;
    unsigned int *ivi;
...
int nd_sp_f4_s(int m,int trace,ND_pairs l,PGeoBucket bucket,SIG *minsig)
{
    ND_pairs t;
    int stat;
    ND spol;
...
int nd_sp_f4_array(int m,int trace,ND_pairs *l,PGeoBucket bucket,SIG *minsig)
{
  ND_pairs t;
  int stat,i;
  ND spol,left,right;
  ...
  min = 0;
  for ( i = 0; i < nd_nbase; i++ ) {
    for ( t = l[i]; t; t = NEXT(t) ) {
#if 0
      stat = nd_sp_lr(m,trace,t,&left,&right);
      if ( !stat ) return 0;
      add_pbucket_symbolic(bucket,left);
      add_pbucket_symbolic(bucket,right);
#else
      stat = nd_sp(m,trace,t,&spol);
      if ( !stat ) return 0;
      if ( spol )
...
int nd_sp_f4_array_nms(int m,int trace,ND_pairs *l,SGeoBucket bucket,SIG *minsig)
{
  ND_pairs t;
  int stat,i;
  ND spol;
  ...
  for ( i = 0; i < nd_nbase; i++ ) {
    for ( t = l[i]; t; t = NEXT(t) ) {
      ...
      if ( !stat ) return 0;
      if ( spol )
        add_sbucket(bucket,nd_to_nms(spol));
      if ( min == 0 || comp_sig(min,t->sig) > 0 ) 
        min = t->sig;
    }
  }
...
//      sugarh = (int)ZTOS((Q)ARG0(node));
  ...
  fprintf(asir_out,"number of removed pairs=%d=(B:%d,MF:%d,2:%d)\n",NcriB+NcriMF+Ncri2,NcriB,NcriMF,Ncri2);
//  fprintf(asir_out,"=(%d+%d+%d)/%d\n",n_bfo,n_cfo,n_dfo,n_afo);
  }
  conv_ilist(nd_demand,0,g,indp);
  return g;
}
...
NODE nd_f4_red(int m,ND_pairs sp0,int trace,UINT *s0vect,int col,NODE rp0,ND_pairs *nz)
{
    ...
    if ( m > 0 )
        ...
        if ( nd_thread )
          ...
          r0 = nd_f4_red_small_main(m,sp0,nsp,s0vect,col,rvect,rhead,imat,nred,nz);
#endif
        else
        ...
        NM_ind_pair *rvect,int *rhead,IndArray *imat,int nred,ND_pairs *nz)
{
    ...
    UINT *svect,*v;
    int *colstat;
    struct oEGT eg0,eg1,eg2,eg_f4,eg_f4_1,eg_f4_2,eg3,eg_spconv;
    int maxrs;
    int *spsugar;
    ND_pairs *spactive;
   ...
   init_eg(&eg_spconv);
    ...
    get_eg(&eg2);
        ...
        if ( !spol ) continue;
        nd_to_vect(m,s0vect,col,spol,svect);
    get_eg(&eg3); add_eg(&eg_spconv,&eg2,&eg3);
        maxrs = ndv_reduce_vect_small(m,svect,col,imat,rvect,nred,0);
        for ( i = 0; i < col; i++ ) if ( svect[i] ) break;
        if ( i < col ) {
            spmat[sprow] = v = (UINT *)MALLOC_ATOMIC(spcol*sizeof(UINT));
            for ( j = k = 0; j < col; j++ )
    ...
    if ( DP_Print ) {
        fprintf(asir_out,"elim1=%.3fsec(spconv=%.3fsec),",eg_f4_1.exectime,eg_spconv.exectime);
        fflush(asir_out);
    }
    /* free index arrays */
    for ( i = 0; i < nred; i++ ) GCFREE(imat[i]->index.c);
...
void print_indarray(IndArray ia,UINT *s0vect)
{
  int i,j,k,len,pos,prev;
  unsigned char *ivc;
  unsigned short *ivs;
  ...
  k = ia->head; 
  redv = nd_ps[ia->number];
  len = LEN(redv); mr = BDY(redv);
  prev = k;
  printf("%ld*",CM(mr));
  ndl_print(s0vect+prev*nd_wpd);
  ...
  switch ( ia->width ) {
    case 1:
      ...
      for ( j = 1, NMV_ADV(mr); j < len; j++, NMV_ADV(mr) ) {
        pos = prev+ivc[j]; prev = pos;
        printf("%ld*",CM(mr)); ndl_print(s0vect+prev*nd_wpd);
      }
      break;
    case 2:
      ivs = ia->index.s; 
...
void ndv_reduce_vect64_by_redlist(int m,mp_limb_t *svect,mp_limb_t *cvect,int col,NODE rp0,SIG sig)
{
  ...
  for ( rp = rp0; rp; rp = NEXT(rp) ) {
    ivect = (IndArray)BDY(rp);
    k = ivect->head; 
    a = svect[k]; c = cvect[k];
    MOD128(a,c,m);
    svect[k] = a; cvect[k] = 0;
    if ( (c = svect[k]) != 0 && (sig == 0 || comp_sig(sig,ivect->sig) > 0 ) ) {
      Nf4_red++;
      c = m-c; redv = nd_ps[ivect->number];
      len = LEN(redv); mr = BDY(redv);
      svect[k] = 0; prev = k;
      switch ( ivect->width ) {
        case 1:
...
void ndv_reduce_vect64_by_redarray(int m,mp_limb_t *svect,mp_limb_t *cvect,int col,IndArray *redarray,SIG sig)
{
  int i,j,k,len,pos,prev;
  mp_limb_t a,c,c1,c2;
  IndArray ivect;
...
int ndl_find_reducer_sigrange(UINT *dg,SIG minsig)
{
  RHist r;
  long i;
  int imin,k,d,c;
...
int ndl_find_reducer_nms(NMs nms)
{
  UINT *dg;
  SIG minsig;
  RHist r;
...
struct oEGT eg_find,eg_sig,eg_smul,eg_sadd;
int nd_symbolic_preproc_s(PGeoBucket bucket,int trace,SIG minsig,UINT **s0vect,NODE *r)
{
  NODE rp0,rp;
  NM mul,head,s0,s;
  long index,index1;
...
//  printf("\n");
  ...
  while ( 1 ) {
    ...
    s = head;
get_eg(&eg1);
    index = ndl_find_reducer_sigrange(DL(head),minsig);
get_eg(&eg2); add_eg(&eg_find,&eg1,&eg2);
    if ( index >= 0 && index < nd_psn ) {
//      ndl_print(DL(head)); ndl_print(DL(nd_psh[index])); printf("%ld\n",index);
      h = nd_psh[index];
      NEWNM(mul);
      ndl_sub(DL(head),DL(h),DL(mul));
      if ( ndl_check_bound2(index,DL(mul)) ) 
...
get_eg(&eg1);
      ...
      pair->head = head;
      red = ndv_mul_nm_symbolic(mul,ps[index]);
get_eg(&eg2); add_eg(&eg_smul,&eg1,&eg2);
get_eg(&eg1);
      add_pbucket_symbolic(bucket,nd_remove_head(red));
get_eg(&eg2); add_eg(&eg_sadd,&eg1,&eg2);
      NEXTNODE(rp0,rp); BDY(rp) = (pointer)pair;
    }
    col++;
  }
...
int nd_symbolic_preproc_nms(SGeoBucket bucket,int trace,SIG minsig,UINT **s0vect,NODE *r)
{
  NODE rp0,rp;
  NMs head,s0,s;
  NM mul,head1;
  ...
  if ( nd_demand )
    ...
    ps = trace?nd_ps_trace:nd_ps;
  while ( 1 ) {
    head = remove_head_sbucket(bucket);
    if ( !head ) break;
    if ( !s0 ) s0 = head;
    else NEXT(s) = head;
    s = head;
//    ndl_print(DL(head)); printf("\n");
get_eg(&eg1);
    index = ndl_find_reducer_nms(head);
get_eg(&eg2); add_eg(&eg_find,&eg1,&eg2);
    if ( index >= 0 && index < nd_psn ) {
//      ndl_print(DL(head)); ndl_print(DL(nd_psh[index])); printf("%ld\n",index);
      h = nd_psh[index];
      ...
      MKNM_ind_pair(pair,mul,index,sugar,sig);
      NEWNM(head1);
      NEXT(head1)=0; ndl_copy(DL(head),DL(head1)); CM(head1)=1;
      pair->head = head1;
      red = ndv_mul_nm_symbolic(mul,ps[index]);
      // ndl_print(DL(head1)); // ndl_print(DL(mul)); nd_print(red); printf("\n");
      red->sig = head->sig;
      add_sbucket(bucket,nd_to_nms(nd_remove_head(red)));
      NEXTNODE(rp0,rp); BDY(rp) = (pointer)pair;
    }
    col++;
  }
...
//  union oNDC hc;
  ...
  SIG sig,minsig;
  struct oEGT eg1;
  struct oEGT eg2,eg_remove,eg_nf,eg_nfzero,eg_symb,eg_find,eg_smul,eg_sadd;

  init_eg(&eg_symb); init_eg(&eg_find); init_eg(&eg_smul); init_eg(&eg_sadd);
  Nf4_red=0;
  d = 0;
  syzlist = (NODE *)MALLOC(nd_psn*sizeof(NODE));
  for ( i = 0; i < nd_psn; i++ ) {
...
//      sugar = nd_sugarweight?d->sugar2:SG(d);
      bucket = create_pbucket();
      stat = nd_sp_f4_s(m,0,l,bucket,&minsig);
      if ( !stat ) {
        for ( t = l; NEXT(t); t = NEXT(t) );
        NEXT(t) = d; d = l;
        d = nd_reconstruct(0,d);
...
NODE ia_insert_or_replace(NODE rp0,IndArray ia)
{
  int head;
  IndArray tia;
  NODE prev,t,nd;
...
int merge_dl_vect(UINT *s0vect,int col0,UINT *s1vect,int col1,UINT **s2vect)
{
  int col2,i,j,k,c;
  UINT *s2v,*p0,*p1,*p2;

  col2 = col0 + col1;
  s2v = (UINT *)MALLOC_ATOMIC(col2*nd_wpd*sizeof(UINT));
  i = 0; j = 0; k = 0;
  p0 = s0vect; p1 = s1vect; p2 = s2v;
  while ( i < col0 && j < col1 ) {
    c = DL_COMPARE(p0,p1);
    if ( c > 0 ) {
      ndl_copy(p0,p2); p0 += nd_wpd; i++;
    } else if ( c < 0 ) {
      ndl_copy(p1,p2); p1 += nd_wpd; j++;
    } else {
      ndl_copy(p0,p2); p0 += nd_wpd; p1 += nd_wpd; i++; j++;
    }
    k++; p2 += nd_wpd;
  }
  if ( i == col0 ) {
    for ( ; j < col1; p1 += nd_wpd, p2 += nd_wpd, j++, k++ )
      ndl_copy(p1,p2);
  } else {
    for ( ; i < col0; p0 += nd_wpd, p2 += nd_wpd, i++, k++ )
      ndl_copy(p0,p2);
  }
  *s2vect = s2v;
  return k;
...
NODE merge_ind_list(NODE rp0,NODE rp1)
{
  struct oNODE root;
  NODE r,t;
  NM_ind_pair p0,p1;
  ...
  r = &root;
  while ( rp0 && rp1 ) {
    p0 = (NM_ind_pair)BDY(rp0);    
    p1 = (NM_ind_pair)BDY(rp1);    
    c = DL_COMPARE(DL(p0->head),DL(p1->head));
    if ( c > 0 ) {
      MKNODE(t,(pointer)BDY(rp0),0); NEXT(r) = t; r = t;
      rp0 = NEXT(rp0);
    } else if ( c < 0 ) {
      MKNODE(t,(pointer)BDY(rp1),0); NEXT(r) = t; r = t;
      rp1 = NEXT(rp1);
    } else {
      c = comp_sig(p0->sig,p1->sig);
      if ( c < 0 ) {
        MKNODE(t,(pointer)BDY(rp0),0); NEXT(r) = t; r = t;
        rp0 = NEXT(rp0);
      } else if ( c > 0 ) {
...
IndArray *redlist_to_array(NODE rp0,int col)
{
  IndArray *r;
  IndArray ia;
...
init_eg(&eg_ia); init_eg(&eg_sp); init_eg(&eg_symb); eg_reducible=0;
  syzlist = (NODE *)MALLOC(nd_psn*sizeof(NODE));
  d = (ND_pairs *)MALLOC(nd_psn*sizeof(ND_pairs));
  nd_nbase = nd_psn;
  Nsyz = 0;
...
again :
  ...
  get_eg(&eg2); add_eg(&eg_symb,&eg1,&eg2);
    ...
    // inita reducer list expressed by IndexArray
  get_eg(&eg1);
    printf("number of reducers=%d\n",length(rp0));
    redlist = conv_ind_pairs(rp0,s0vect,col);
    redarray = redlist_to_array(redlist,col);
...
// printf("(%d,%d)",l->i1,l->i2); print_sig(l->sig); printf("\n");
      ...
      if ( nd_to_vect64(m,s0vect,col,h,svect) < 0 ) {
        int col1;
        UINT *s1vect,*s2vect;
        NODE rp1;
...
//      ndv_reduce_vect64_by_redlist(m,svect,cvect,col,redlist,sig);
      ndv_reduce_vect64_by_redarray(m,svect,cvect,col,redarray,sig);
      for ( i = 0; i < col; i++ ) if ( svect[i] ) break;
      if ( i < col ) {
        dmin[ind] = dmin[ind]->next; dlen--;
        Nnfnz++;
...
//        if ( j && DP_Print ) printf("<%d>",j);
        ...
        ia->number = nh;
        ia->mul = mul;
        redlist = ia_insert_or_replace(redlist,ia);
        redarray = redlist_to_array(redlist,col);
      } else {
        dmin[ind] = dmin[ind]->next; dlen--;
        Nnfz++;
...
init_eg(&eg_find); init_eg(&eg_sig);
init_eg(&eg_ia); init_eg(&eg_sp); init_eg(&eg_symb); eg_reducible=0;
  syzlist = (NODE *)MALLOC(nd_psn*sizeof(NODE));
  d = (ND_pairs *)MALLOC(nd_psn*sizeof(ND_pairs));
  nd_nbase = nd_psn;
...
again :
    ...
    for ( i = 0; i < nd_nbase; i++ )
      for ( j = 0, t = dmin[i]; t; t = NEXT(t), k++ );
    printf("\nsugar=%d:number of spairs=%d\n",sugar,k);
    sbucket = create_sbucket();
  get_eg(&eg1);
    stat = nd_sp_f4_array_nms(m,0,dmin,sbucket,&minsig);
  get_eg(&eg2); add_eg(&eg_sp,&eg1,&eg2);
    if ( !stat ) {
      for ( i = 0; i < nd_nbase; i++ ) {
        t = dmin[i];
    ...
    if ( sbucket->m < 0 ) continue;
  get_eg(&eg1);
    col = nd_symbolic_preproc_nms(sbucket,0,minsig,&s0vect,&rp0);
  get_eg(&eg2); add_eg(&eg_symb,&eg1,&eg2);
    if ( !col ) {
      for ( i = 0; i < nd_nbase; i++ ) {
        t = dmin[i];
...
// printf("(%d,%d)",l->i1,l->i2); print_sig(l->sig); printf("\n");
      ...
      if ( nd_to_vect64(m,s0vect,col,h,svect) < 0 ) {
        ...
        printf("reconstruct redlist\n");
        sbucket = create_sbucket();
        add_sbucket(sbucket,nd_to_nms(h));
        col1 = nd_symbolic_preproc_nms(sbucket,0,minsig,&s1vect,&rp1);
        rp0 = merge_ind_list(rp0,rp1);
        printf("col : %d->",col);
        col = merge_dl_vect(s0vect,col,s1vect,col1,&s2vect); 
        printf("%d\n",col);
