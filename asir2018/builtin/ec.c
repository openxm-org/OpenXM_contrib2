...
void Psha1(NODE arg,Z *rp)
{
  ...
  z = (Z)ARG0(arg);
  s = (unsigned char *)mpz_export(0,&bl,1,sizeof(unsigned char),1,0,BDY(z));
  sha_memory(s,bl,digest);
  mpz_init(r);
  mpz_import(r,5,1,sizeof(int),0,0,digest);
  MPZTOZ(r,*rp);
}
...
void ecm_add_ff(VECT p1,VECT p2,VECT ec,VECT *pr)
{
  if ( !p1 )
    *pr = p2;
  else if ( !p2 )
...
void ecm_add_gf2n(VECT p1,VECT p2,VECT ec,VECT *rp)
{
...
unsigned int ecm_count_order_gfp(unsigned int p,unsigned int a,unsigned int b)
{
...
unsigned int ecm_count_order_gf2n(UP2 d,GF2N a,GF2N b)
{
...
void ecm_add_gfp(VECT p1,VECT p2,VECT ec,VECT *pr)
{
...
void ecm_chsgn_ff(VECT p,VECT *pr)
{
  Obj x,y,z;
  LM tl;
  GF2N tg;
...
void ecm_sub_ff(VECT p1,VECT p2,VECT ec,VECT *pr)
{
  VECT mp2;
...
int comp_kip(struct oKeyIndexPair *a,struct oKeyIndexPair *b)
{
  unsigned int ka,kb;
...
void compute_all_key_homo_gfp(VECT *pa,int len,unsigned int *ka)
{
  LM *b,*x,*z;
  int i;
  LM t,s,m;
  ...
  for ( i = len-1; i >= 0; i-- ) {
    ...
    ka[i] = LMTOS(s); ka[i] |= 0x80000000;
    mullm(m,z[i],&s); m = s;
  }
}

void compute_all_key_homo_gf2n(VECT *pa,int len,unsigned int *ka)
{
  GF2N *b,*x,*z;
  int i;
  GF2N t,s,m;
...
unsigned int separate_vect(double *v,int n)
{
...
void ecm_find_match(unsigned int *g,int ng,unsigned int *b,int nb,LIST *r)
{
  int i,j;
  Z iq,jq;
  NODE n0,n1,c0,c;
...
int find_match(unsigned int k,unsigned int *key,int n)
{
  int s,e,m;
...
int nextvect1(VECT vect,VECT bound)
{
  int size,i,a;
  Z *vb,*bb;
...
void sort_ktarray(VECT karray,VECT tarray,LIST *rp)
{
  NODE r,r1;
