========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : send_to_asir
========================================================================


AppWizard が生成した send_to_asir には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには send_to_asir を構成している各ファイルの概要説明が含まれています。
...
send_to_asir.h
    ...
    みます） をインクルードし、また CSend_to_asirApp アプリケーションクラスを宣
    言します。

send_to_asir.cpp
    このファイルは CSend_to_asirApp アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。
...
res\send_to_asir.ico
    このファイルは、アプリケーションのアイコンとして使用されるアイコンファイ
    ルです。このアイコンはリソース ファイル send_to_asir.rc によってインクルード
    されます。

res\send_to_asir.rc2
...
send_to_asirDlg.h, send_to_asirDlg.cpp - ダイアログ
    これらのファイルは CSend_to_asirDlg クラスを含みます。このクラスは 
    アプリケーションのメイン ダイアログの動作を定義します。ダイアログ
    テンプレートは Microsoft Visual C++ で編集可能な send_to_asir.rc に含
    まれます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) send_to_asir.pch 
